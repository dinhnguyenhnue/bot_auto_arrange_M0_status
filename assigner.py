...
def assign_m0_lead_to_tvv(client: LarkClient, lead_record_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not tvv_empty and recipient_empty:
        ...
        if selected:
            ...
            if field_type == 18:
                # Link field (one-way SingleLink) cannot be written via API
                # It will be synced by Lark Automation or the dispatch table link
                logger.info(
                    f"Skipping TVV Link field (type 18) for Lead {lead_record_id} "
                    f"→ {selected['name']} (link_record_id={selected.get('tvv_link_record_id')}). "
                    f"Only filling 'Người nhận data'."
                )
            elif field_type == 11:
                ...
                if selected_tvv:
                    ...
                    if field_type == 18:
                        ...
                        logger.info(
                            ...
                            f"→ {selected_tvv['name']} (link_record_id={selected_tvv.get('tvv_link_record_id')}). "
                            f"Only filling 'Người nhận data'."
                        )
                    elif field_type == 11:
                        update_fields[config.FIELD_TIKTOK_ASSIGNED_USER] = [{"id": recipient_uid}]
